...
class Animator:
    def __init__(self, results_code: Dict):
    ...
    def animate(self, fps: int = 15) -> None:
        """
        ...
        steps_per_move = int(getattr(self.results_code, "routing_steps_per_move", 15))
        pause_frames = int(getattr(self.results_code, "routing_pause_frames", 5))
        n_stages = len(stage_positions)
        frames_per_segment = (steps_per_move + pause_frames) if n_stages > 1 else 0
        z_laser_top = (self.results_code.get("layers", 0) - 1) * self.results_code.get("spacing_z", 0)  # laser from readout plane (z) down to atom

        if not stage_positions:
            raise ValueError("[Error] No stage_positions found. Make sure to run InitialPlacer.solve() first.")
        ...
        output_dir = f"results/{self.results_code['dir']}/animation/"
        os.makedirs(output_dir, exist_ok=True)
        save_path = output_dir + self.results_code['benchmark'] + ".mp4"
        ...
        self._plot_architecture_background(
            ax3d,
            alpha_storage=0.1,
        ...
        n_qubits = self.results_code["n_qubits"]
        ...
        # Axes settings
        # 提前计算好物理边界与留白（margin）
        xy_max = self.results_code.get("size", 0) * self.results_code.get("spacing_xy", 0)
        z_max = self.results_code.get("layers", 0) * self.results_code.get("spacing_z", 0)
        margin_xy = self.results_code.get("spacing_xy", 0) * 0.5
        margin_z = self.results_code.get("spacing_z", 0) * 0.5
        ...
        x_AOD_min = -0.5 * self.results_code.get("spacing_xy", 0)
        x_AOD_max = self.results_code.get("size", 0) * self.results_code.get("spacing_xy", 0) + 0.5 * self.results_code.get("spacing_xy", 0)
        y_AOD_min = -0.5 * self.results_code.get("spacing_xy", 0)
        y_AOD_max = self.results_code.get("size", 0) * self.results_code.get("spacing_xy", 0) + 0.5 * self.results_code.get("spacing_xy", 0)
        ...
        def update(frame_idx: int):
            ...
            ax3d.set_title(f"3D Atom Shuttling: {self.results_code['benchmark']}\nFrame {frame_idx}/{total_frames}")
